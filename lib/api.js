...
/**
 * Mapping from export item key names to properties in `snippet` objects returned by API.
 */
const snippetMapping = {
  position: "position",
  title: "title",
  uploader: "videoOwnerChannelTitle",
  uploaderUrl: "videoOwnerChannelId",
 ...
 */
export async function getPlaylistData(playlistId, exportItems) {
  const config = new Config();
  const apiKey = config.apiKey;

  try {
    const response = await axios(`${API_BASE_URL}/playlistItems`, {
      params: {
        key: apiKey,
        part: "snippet, status",
        playlistId,
    ...
    data.items.forEach(({ snippet, status }) => {
      ...
      exportItems.forEach((item) => {
        let data = getValue(snippet, snippetMapping[item]);
